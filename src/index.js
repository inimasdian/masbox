class Masbox {
	...
	static bind(selector, options = {}) {
		document.querySelectorAll(selector).forEach((el) => {
			...
			const exists = Masbox.bindings.find((b) => b.group === group);
			if (!exists) {
				Masbox.bindings.push({ group, selector, options });
			}
			...
			el.addEventListener('click', (e) => {
				...
				const config = Masbox.bindings.find((b) => b.group === group)?.options ?? {};
				new Masbox(gallery, index, config).open();
			});
		});
	}
	...
	_createItem(platform = null) {
		const linkEl = this.gallery?.[this.currentIndex];
		const image = this.imageEl?.src || linkEl?.href || '';
		const caption = this.captionEl?.textContent || linkEl?.dataset.caption || '';
		const url = window.location.href; // address bar
	...
	updateImageMaxHeight() {
		...
		if (this.options.caption && this.captionEl?.offsetHeight) {
			deduction += this.captionEl.offsetHeight;
		}
	
		if (this.options.thumbnails && this.thumbnails?.offsetHeight) {
    ...
    async open() {
		...
		this._onClickOutside = (e) => {
			...
			const clickedInside = allowedSelectors.some((selector) =>
				e.target.closest(selector)
			);

			// 🔄 Kalau share menu lagi terbuka dan klik di luar menu atau tombol share, close dulu menu-nya
			...
			// ❌ Kalau klik di luar semua, baru tutup viewer
		...
		this.hasZoomed = false;
		this.options.onOpen?.(this._createItem());
    ...
    async close() {
		...
		this.options.onClose?.(this._createItem());
    ...
    buildOverlay() {
		...
		if (this.options.flip) {
			toolbar.querySelector('.flip-horizontal')?.addEventListener('click', () => {
				this.transform.flipH *= -1;
				this.updateTransform();
			});
			
			toolbar.querySelector('.flip-vertical')?.addEventListener('click', () => {
		...
		if (this.options.thumbnails) {
			toolbar.querySelector('.thumbnails')?.addEventListener('click', () => {
		...
		if (this.options.share) {
			...
			buttons.forEach((btn) => {
				...
				btn.onclick = (e) => {
					...
					const image = current.href || current.src || '';
	...
	navigate(step) {
		...
		this.hasZoomed = false;
		this.options.onMove?.(this._createItem());
	}
    ...
    setZoom(value) {
		...
		if (!this.hasZoomed && this.zoom > 1) {
			this.hasZoomed = true;
			this.options.onZoom?.(this._createItem());
		}
	}	
